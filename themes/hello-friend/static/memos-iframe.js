var addCSS = "#toBi iframe{border:1px solid #eaeaea;box-shadow: 0 4px 6px 0px rgba(0,0,0,.14);position: fixed;flex-direction: column;justify-content: space-between;bottom: 5rem;right: 1rem;width: 448px;min-height:269px;height:269px;max-height: 600px;border-radius: 0.45rem;display: flex;z-index: 999999;overflow: hidden;left: unset;animation: atkFadeIn both .3s;background-color: #f6f5f4;}#toBi-icon{position: fixed; bottom: 1rem; right: 1rem; width: 50px; height: 50px; border-radius: 25px; background-color: rgb(0, 0, 0); box-shadow: rgba(0, 0, 0, 0.2) 0px 4px 8px 0px; cursor: pointer; z-index: 2147483645; transition: all 0.2s ease-in-out 0s; left: unset; transform: scale(1);}#toBi-icon:hover{transform: scale(1.08);}#toBi-svg{display: flex; align-items: center; justify-content: center; width: 100%; height: 100%;}@keyframes atkFadeIn {0% {opacity: 0} to {opacity: 1}}@media (max-width:550px) {#toBi iframe{right:0;width:100%;}}"
...
function addIframe(){
    ...
    iframe.src = "https://immmmm.com/memos"; // 替换为您要嵌入的远程网页的 URL
    iframe.id = "memos"
